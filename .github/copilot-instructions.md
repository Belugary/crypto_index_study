...
- **用户视角**: README 是给最终用户看的，内容必须围绕“用户如何用、能得到什么结果”展开。
- **简洁性**: 避免冗余的技术细节和开发者视角，突出核心价值和操作步骤。
- **结构建议**:
  - 开头突出项目能解决什么问题、适合什么场景。
  - 快速开始和常用命令放前面，越简单越好。
  - 详细功能和参数说明放后面，按需查阅。
  - 技术细节、哲学原则、分类逻辑等可以简化为一句话，或放到开发文档而不是用户文档。
- **补充内容**: 如果用户需要运行测试或查看变更日志，建议在 README 中添加相关说明。
