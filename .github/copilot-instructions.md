...
- **测试**: 所有测试必须放在 `tests/` 目录，并覆盖新增的 API 功能。
  - 新增 `tests/test_incremental_daily_updater.py`，覆盖增量每日数据更新核心逻辑。
...
- **核心目录**: `src/` (核心代码), `src/updaters/` (更新器模块), `scripts/` (自动化脚本，例如 `incremental_daily_update.py`), `tests/` (测试), `data/` (数据资产)。
...
- **回滚精确**: 问题定位到具体功能模块
- **审查高效**: 每次变更目的明确，易于代码审查
- **历史清晰**: Git 历史反映真实的开发进程
- **协作友好**: 团队成员容易理解每次变更意图
...
```bash
# ✅ 正确方式 - 分批提交
git add src/index/market_cap_weighted.py
git commit -m "🔧 指数计算器架构重构"

git add src/classification/wrapped_coin_checker.py
git commit -m "🎯 包装币检查器增强"

git add data/daily/daily_files/*.csv
git commit -m "📊 数据更新: 新增每日汇总"

# ❌ 错误方式 - 大杂烩提交
git add -A
git commit -m "修复各种问题和更新数据"  # 模糊不清
```
