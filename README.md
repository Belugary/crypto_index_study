...
│   └── utils/             # 工具函数
├── scripts/               # 执行脚本
│   ├── update_all_metadata.py   # 元数据更新
│   └── update_price_data.py     # 价格数据更新
├── data/                  # 数据存储
│   ├── coins/             # 币种历史数据 (CSV)
│   └── metadata/          # 元数据文件
├── logs/                  # 日志文件
...
- **CoinGecko API 客户端**: 封装 Pro API 调用，支持速率限制和错误处理
- **批量下载器**: 智能批量下载和管理历史数据
- **增量更新系统**: 自动检测和更新过期数据
- **元数据管理**: 维护币种信息和下载状态
...
| `update_price_data()`       | 增量更新   | 智能更新过期的价格数据         |
...
- API 请求成功返回数据
- 数据格式验证通过
- CSV 文件成功保存到 `data/coins/` 目录

#### ⏭️ **skipped** (智能跳过)
- 本地已有足够新鲜的数据
- 时间阈值规则：
  - `days="max"` (完整历史): 24 小时内认为新鲜
  - 具体天数参数: 12 小时内认为新鲜

#### ❌ **failed** (下载失败)
- 可能原因：网络问题、API 限制、币种不存在等
- 自动重试 3 次，每次间隔 5 秒
...
2. 调整请求间隔参数
3. 分批下载，避免速率限制
4. 查看日志文件: `logs/batch_download.log`

### 📁 相关文件

- **数据文件**: `data/coins/*.csv`
- **元数据**: `data/metadata/download_metadata.json`
- **日志文件**: `logs/batch_download.log`
- **失败记录**: `logs/failed_downloads.log`
