...
- **历史数据资产**: 包含市值前 600 名加密货币的完整历史量价数据 (最近更新: 2025-07-11)
  - 包含价格、交易量、市值等关键指标
  - 日级别数据粒度，完整历史覆盖
  - 最终获取 638 个币种，筛选后得到 **534 个原生币**
- **灵活的原生币数量配置**: 支持自定义目标原生币数量（默认 510 个）
  - 动态搜索范围扩展，自动调整直到找到足够的原生币
  - 智能增量策略，根据缺口大小采用不同增量（500/300/200）
  - 避免固定搜索范围限制，确保目标达成
- **智能资产分类 (基于“简单有效”哲学)**: 准确识别并分类稳定币和包装币
  - **信任权威数据源**: 严格遵循 CoinGecko 官方分类，摒弃了过度工程化的自定义逻辑，从根本上保证了分类的准确性。
  - **稳定币识别**: 仅使用 `Stablecoins` 官方分类，成功识别 80 个稳定币。
  - **包装币识别**: 仅使用 `Wrapped-Tokens` 官方分类，成功识别 24 个包装币。
- **智能增量更新**: 自动检测并更新过期数据，高效维护数据完整性
- **数据过滤功能**: 支持在指数研究中过滤掉稳定币和包装币，专注原生资产
  - 过滤效果：从 638 个币种中自动识别并排除 104 个非原生资产 (80 个稳定币 + 24 个包装币)，最终获得 534 个原生币。
- **模块化设计**: 便于扩展和定制化研究
- **中文文档**: 详细的使用说明和代码注释
...
| `update_price_data.py`   | 价格更新   | 增量更新原生币价格数据，支持自定义目标数量（默认 510 个） |
...
# 查看所有可用参数
python scripts/update_price_data.py --help
```
...
- **包装代币**：仅使用 "Wrapped-Tokens" 官方类别
- **精确分类**：成功识别 24 个包装币，避免过度分类
- **质量保证**：排除了之前误判的质押代币、桥接代币等，确保分类准确性
