...
class BatchDownloader:
    ...
    def download_batch(
        ...
        try:
            ...
            self.logger.info(f"批量下载完成 - 成功: {success_count}, 跳过: {skipped_count}, 失败: {failed_count}")
    ...
    def _ensure_directories(self) -> None:
        ...
        "显式胜于隐式" - 明确创建所需的目录结构
        """
        for directory in [self.data_dir, self.coins_dir, self.metadata_dir, self.logs_dir]:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        file_handler = logging.FileHandler(
            self.logs_dir / "batch_download.log",
            encoding='utf-8'
        )
...
def create_batch_downloader(api_key: Optional[str] = None, data_dir: str = "data") -> BatchDownloader:
    """
    创建批量下载器的便捷函数
