...
from src.updaters.price_updater import CoinClassifier, MarketDataFetcher, PriceDataUpdater
...
class TestPriceDataUpdater(unittest.TestCase):
    ...
    def setUp(self):
        """设置测试环境"""
        with patch('src.updaters.price_updater.CoinGeckoAPI'), \
...
class TestMetadataUpdater(unittest.TestCase):
    ...
    def setUp(self):
        """设置测试环境"""
        with patch('src.updaters.metadata_updater.StablecoinChecker'), \
             patch('src.updaters.metadata_updater.WrappedCoinChecker'), \
